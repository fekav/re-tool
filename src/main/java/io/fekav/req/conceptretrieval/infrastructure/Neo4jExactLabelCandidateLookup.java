...
@ApplicationScoped
public class Neo4jExactLabelCandidateLookup implements ConceptCandidateLookup {

    private static final String QUERY = """
        MATCH (candidate)
        WHERE (candidate.label = $text OR candidate.text = $text)
    ...
    @Inject
    public Neo4jExactLabelCandidateLookup(Driver driver) {
        this.driver = driver;
    }

    @Override
    public String lookupMethodName() {
        return OrderedWeightedConceptRetrievalPolicy.EXACT_LABEL_LOOKUP;
    }

    @Override
    public double lookupWeight() {
        return OrderedWeightedConceptRetrievalPolicy.EXACT_LABEL_WEIGHT;
    }
    ...
    private List<CandidateLookupHit> lookupHits(
        ...
        for (CandidateConcept candidate : candidates) {
            hits.add(new CandidateLookupHit(
                candidate,
                lookupMethodName(),
                "Matched exact label '" + selectedTerm.text() + "' to graph candidate '" +
                    candidate.label() + "'"
            ));
        }
