...
| `RETRIEVE_CANDIDATE_CONCEPTS` | Produce ranked KG concept candidates or explicit no-match evidence for each syntax element selected for matching. | `Requirement.ElementId`; selected `Action` element ids and text values. | `retrieve_candidate_concepts(selectedElements)` | `CandidateConceptMatchSet` with retrieval evidence for every selected element. | `CONCEPT_RETRIEVAL_POLICY`: hybrid approach with graph traversal, alias queries, similarity search with embeddings/vectors   |
