...
| `CLASSIFY_REQUIREMENT` | Identify the requirement's initial KG type, property, confidence, and rationale. | `RequirementRecord`; allowed requirement concept types and properties. | Classify as `Goal`, `Need`, or `Requirement`; classify property as functional or quality when eligible; assign confidence score; attach rationale and context tags if available. | `RequirementClassification` linked to `RequirementRecord`. | No allowed requirement concept type exists in KG configuration. |
