...
| `RETRIEVE_CANDIDATE_CONCEPTS` | Produce ranked KG concept candidates or explicit no-match evidence for each syntax element selected for matching. | Selected syntax term roles and text values. Command callers do not provide ids. | `retrieve_candidate_concepts(selectedTerms)` | `CandidateConceptMatchSet` with retrieval evidence for every selected term. | `CONCEPT_RETRIEVAL_POLICY`: v1 `exactMatch` retrieves explicit graph candidates by exact text/label equality; later policies may add graph traversal, alias queries, or embedding similarity. |
...
| `CandidateConceptMatchSet` | `RETRIEVE_CANDIDATE_CONCEPTS` | `CREATE_MATCHING_CANDIDATES` | Structured collection linked to selected term role/text values | Every selected term has zero or more ranked matches and retrieval evidence. |
