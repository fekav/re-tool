...
| `RETRIEVE_CANDIDATE_CONCEPTS` | Produce ranked KG concept candidates or explicit no-match evidence for each syntax element selected for matching. | `Requirement.ElementId`; selected `Action` element ids and text values. | `retrieve_candidate_concepts(selectedElements)` | `CandidateConceptMatchSet` with retrieval evidence for every selected element. | `CONCEPT_RETRIEVAL_POLICY`: v1 `exactMatch` retrieves concepts by exact label equality; later policies may add graph traversal, alias queries, or embedding similarity. |
...
| `CandidateConceptMatchSet` | `RETRIEVE_CANDIDATE_CONCEPTS` | `CREATE_MATCHING_CANDIDATES` | Structured collection linked to term mention ids | Every selected element has zero or more ranked matches and retrieval evidence. |
