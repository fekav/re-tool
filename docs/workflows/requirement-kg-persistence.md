...
## Purpose
Persist textual requirements into a knowledge graph while preserving provenance, extracting requirement syntax, classifying requirements, retrieving concept-name KG candidates for selected syntax terms, and deciding concept matches. The current concept retrieval slice treats graph labels, text values, aliases, and alias lists as equivalent concept-name matches.
...
| `SelectedTerm` | `RETRIEVE_CANDIDATE_CONCEPTS` | `ConceptRetrievalService`; `ConceptRetrievalPolicy`; `CandidateLookup`; `DECIDE_CONCEPT_MATCHES` | Domain value object with `syntaxRole` and `text` | `syntaxRole` and `text` are non-blank after trimming; duplicate command inputs are rejected before retrieval or decision. |
...
| `NewConceptProposal` | `DECIDE_CONCEPT_MATCHES` | `PERSIST_GRAPH_CHANGES` | Proposed new KG concept produced for lookup misses | Label and concept type are non-blank; v1 derives them from selected term text and syntax role. |
