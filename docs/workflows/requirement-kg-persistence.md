...
|---|---|---|---|---|---|
| `INTAKE_REQUIREMENT` (START) | Capture the raw test of  and its provenance. | Raw requirement text; source identifier; submitter or ingestion actor. | Validate non-empty text; assign requirement id; record source metadata; normalize line endings. | `Provenance` with `RawText`, `sourceIdentifier`, `ingestedAt`. | Raw text or source identifier is missing. |
| `CLASSIFY_REQUIREMENT` | Identify the requirement's initial KG type, property, confidence, and rationale. | `Provenance`; allowed requirement concept types and properties. | Classify as `Goal`, `Need`, or `Requirement`; classify property as functional or quality when eligible; assign confidence score; attach rationale and context tags if available. | `RequirementClassification` linked to `Provenance`. | No allowed requirement concept type exists in KG configuration. |
...
| `CLASSIFY_REQUIREMENT` | Requirement type is one of the configured classification concept types, property is one of the configured requirement properties, confidence score is valid, and rationale is present. | `EXTRACT_REQUIREMENT_SYNTAX` | No | V1 classification concept types are `Goal`, `Need`, and `Requirement`. V1 properties are functional and quality. |
...
| `RequirementClassification` | `CLASSIFY_REQUIREMENT` | `PERSIST_GRAPH_CHANGES` | KG relationship or transaction payload | Type is one of the configured classification concept types, property is one of the configured requirement properties, confidence score is between `0.0` and `1.0`, and rationale is non-blank. |
...
Low confidence is a consumer review and triage signal, not a classifier failure. V1 still returns the best-fit `Goal`, `Need`, or `Requirement` classification because it does not define an `UNKNOWN` concept type.
