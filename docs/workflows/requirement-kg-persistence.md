...
- Retrieving ordered, weighted candidate concepts from the KG or explicit no-match evidence.
...
| `RETRIEVE_CANDIDATE_CONCEPTS` | Produce ordered, weighted KG concept candidates or explicit no-match evidence for each syntax element selected for matching. | Selected syntax term roles and text values. Command callers do not provide ids. | `retrieve_candidate_concepts(selectedTerms)` | `CandidateConceptMatchSet` with retrieval evidence for every selected term. | `CONCEPT_RETRIEVAL_POLICY`: v1 uses `OrderedWeightedConceptRetrievalPolicy`; lookup methods run by descending weight; `exactLabel` has weight `1.0` and stops further lookup on hit; `alias` has weight `0.7`; future non-exact lookup combinations must stay below score `1.0`. |
| `CREATE_MATCHING_CANDIDATES` | Interpret candidate retrieval evidence into mapping outcomes, review items, and concept proposals. | `CandidateConceptMatchSet` | `create_matching_candidates(candidateConceptMatchSet)` | TBD | `MATCHING_POLICY` : exact, probable, ambiguous, and no-match evidence resolve to approve, reject, propose, create, review, or block. |
...
| `RETRIEVE_CANDIDATE_CONCEPTS` | `CONCEPT_RETRIEVAL_POLICY`: every selected element has ordered weighted candidate concepts or explicit no-match evidence. | `CREATE_MATCHING_CANDIDATES` |
| `CREATE_MATCHING_CANDIDATES` | `MATCHING_POLICY`: every selected evidence entry has an automatic mapping outcome, for example `matchConfidence >= autoMapThreshold` creates a match or `matchConfidence < rejectThreshold` rejects the match. | `PERSIST_GRAPH_CHANGES` |
...
| `CandidateConceptMatchSet` | `RETRIEVE_CANDIDATE_CONCEPTS` | `CREATE_MATCHING_CANDIDATES` | Structured collection linked to selected term role/text values | Every selected term has zero or more ordered retrieved candidates and non-empty term-level evidence; every real retrieved candidate has candidate-specific evidence with a required score equal to the sum of applied lookup method weights. |
...
- Abort if the active concept retrieval policy has invalid lookup weights, including non-exact lookup combinations that can reach or exceed score `1.0`.
...
| V1 candidate retrieval uses an ordered weighted retrieval policy: `exactLabel` weight is `1.0`, `alias` weight is `0.7`, exact label hits stop further lookup, and non-exact lookup combinations must remain below `1.0`. | Retrieve candidate concepts implementation plan. | MEDIUM | Retrieval ordering and later matching thresholds could become inconsistent if lookup scores are interpreted differently. | Verify `CandidateConceptMatchSet` scores are produced by `CONCEPT_RETRIEVAL_POLICY` from lookup method weights. |
